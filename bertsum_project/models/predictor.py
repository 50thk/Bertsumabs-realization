...
class Translator(object):
    ...
    def from_batch(self, translation_batch):
        ...
        for b in range(batch_size):
            pred_sents = [self.vocab.idx_to_token[int(n)] for n in preds[b][0]]
            pred_sents = "".join(pred_sents).replace(" ▁", "")
            # gold_sent = "".join(tgt_str[b]).replace("▁", " ")
            ...
            raw_src = [self.vocab.idx_to_token[int(t)] for t in src[b]][:500]
            raw_src = ''.join(raw_src)
            translation = (pred_sents, raw_src) # center gold_sent
    ...
    def translate(self,
                  data_iter, step,
                  attn_debug=False, return_results=True):
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, src = trans # center gold
                    pred_str = (pred.replace('[BOS]', '')
                        .replace('[EOS]', '')
                        .replace('[PAD]', '')
                        .replace('[UNK]', '')
                        .replace('[SEP]', '')
    ...
    def _fast_translate_batch(self,
        ...
        for step in range(max_length):
            ...
            if(self.args.block_trigram):
                ...
                if(cur_len>3):
                    for i in range(alive_seq.size(0)):
                        ...
                        words = [self.vocab.idx_to_token[w] for w in words]
            ...
            topk_beam_index = topk_ids.div(vocab_size, rounding_mode='trunc') #add rounding_mode='trunc'
