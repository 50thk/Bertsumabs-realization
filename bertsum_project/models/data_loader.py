...
class Batch(object):
    ...
    def __init__(self, data=None, device=None, is_test=False):
        ...
        if data is not None:
            ...
            pre_segs = [x[2] for x in data] # x[2]
            pre_clss = [x[3] for x in data] # x[3]
            pre_src_sent_labels = [x[4] for x in data] # x[4]

            src = torch.tensor(self._pad(pre_src, 1))
            tgt = torch.tensor(self._pad(pre_tgt, 1))

            segs = torch.tensor(self._pad(pre_segs, 0))
            mask_src = ~(src == 1) # 1 - (src == 1) -> mask_src = ~(src == 1)
            mask_tgt = ~(tgt == 1) # 1 - (tgt == 1) -> mask_tgt = ~(tgt == 1)

            clss = torch.tensor(self._pad(pre_clss, -1))
            src_sent_labels = torch.tensor(self._pad(pre_src_sent_labels, 0))
            mask_cls = ~(clss == -1) # 1 - (clss == -1) -> ~(clss == -1)
            clss[clss == -1] = 0
