...
def get_kobert_vocab(cachedir="./tmp/"):
    # Add BOS,EOS vocab
    vocab_file = get_tokenizer(cachedir=cachedir)
...
class BertData:
    ...
    def preprocess(self, data, is_test=False):
        data = split_sentences(data.replace("\n", '').replace("\r", ''))
        src = [self.tokenizer(s) for s in data]
        ...
        b_data_dict = {"src": src_subtoken_idxs, "tgt": src_subtoken_idxs, "src_sent_labels": sent_labels, 
        				"segs": segments_ids, 'clss': cls_ids, 'src_txt': src, 'tgt_txt': src}
        return [b_data_dict]
...
def format_to_bert(args):
    ...
    for corpus_type in datasets:
        ...
        for json_str in jsonl:
            data.extend(json.loads(json_str)) # append -> extend
